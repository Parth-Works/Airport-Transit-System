function [A,Q1,Q2,MU,T1,T2,T] = withAQ(lambda,mu1,mu2,Threshold,endtime)
...
%         figure(100),subplot(4,3,5), plot(T1), hold on, title('Sojourn time PHASE 1');
        ...
        if currcustomers1>0
%             if currcustomers1-1>15
%                 event(2) = t + (0.00001+0.00001*randi([1 5]));
%             end
             if queue2 < Threshold
               mu = mu1;
               event(2) = event(2) + exprnd(1/mu);
            else
                mu = mu1*(1/log(currcustomers2));
                event(2) = event(2) + exprnd(1/mu);
            end
